...
function NavItem({ href, text }: any) {
  ...
  return (
    <Link href={href}>
      <a className={classnames(isActive ? "border-indigo-500 border-b-2 font-semibold" : "", "text-gray-900 inline-flex items-center px-1 pt-1 text-base")}>{text}</a>
    </Link>
  );
}
        ...
        <div className="relative flex justify-between h-16">
          ...
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
              ...
              <NavItem href="/protected" text="Protected" />
              <NavItem href="/me" text="Me" />
              <NavItem href="/login" text="Sign in" />
              <NavItem href="/logout" text="Sign out" />
            </div>
          </div>
        </div>
      </div>
